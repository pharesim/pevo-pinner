...
- **`IsPinned` error swallow in `AutoPinRunner.Run` directly violates CLAUDE.md "Swallowed errors cause pin storms" rule.** The line `already, _ := r.backend.IsPinned(ctx, item.CID)` in the runner discards the error. For `PinataBackend`, `IsPinned` is a live HTTP GET that can return `(false, err)` on a transient Pinata failure (429 / 5xx / network blip); the discarded error coerces the answer to "not pinned" and the goroutine falls through to call `Pin` unconditionally. Every refresh cycle re-evaluates the same CID and triggers another `Pin` attempt — pin storm against a backend that is already under stress. Six independent reviewers in the 2026-05-21 architect review (correctness P1, security P1, adversarial P2/conf90, maintainability P1, reliability P0, project-standards violation) flagged this. The pattern was carried forward verbatim from the pre-task `main.go` callback, but the runner is the canonical autopin path going forward and is the right place to fix it. Capture the error: on `err != nil`, log via `[autopin] IsPinned failed for <cid>: <err>` and skip the `Pin` call (count as `Failed`, or add a separate `IsPinnedErrors` counter to `AutoPinResult`). Add a `fakeBackend.IsPinned` error-injection hook and a regression test asserting no `Pin` call follows a failed `IsPinned`.

- **`handlePinAll` in `server.go` bypasses the AutoPinRunner's bounded pool and per-author cap.** The handler iterates `discovery.Items()` and calls `s.backend.Pin(ctx, item.CID)` serially with no concurrency bound, no per-author cap, and no `IsPinned` pre-check. The same wedge attack this task defends against on the autopin callback path is reachable through this management endpoint. The server binds to 127.0.0.1 so external attackers cannot reach directly, but an operator triggering `POST /api/pin-all` from the UI on a hostile-batch discovery set walks into the original 14-day stall this task closed. Three reviewers (security, performance, reliability) flagged it. Route `handlePinAll` through `s.autopinRunner.Run(ctx, items)` the same way `handleEvaluateRules` already does — the runner is wired into the `Server` struct and ready to use — and update the response shape to expose `matched / pinned / failed / shed` from `AutoPinResult` instead of the current `{pinned, failed}` map.

- **`AutoPinRunner.Run`'s concurrency bound is per-`Run`, not global.** The semaphore channel and `WaitGroup` are allocated as stack locals inside `Run`, and the runner doc says "Run is safe for serial reuse, not for concurrent batches" — yet the production wiring exposes two concurrent callers of the same runner instance: the `SetOnRefresh` callback in `main.go` and the HTTP handler `handleEvaluateRules` in `server.go`. Two simultaneous invocations (an operator clicking Evaluate-Rules while a discovery refresh is mid-flight) produce effective in-flight = 2 × `AUTOPIN_CONCURRENCY`, exceeding the documented bound. Recommended fix: promote the semaphore channel to a `runner` struct field so it caps in-flight pins across all callers; the `WaitGroup` can stay local. Alternatively, take a `runner.mu` mutex around the body of `Run` to serialize calls. Either way, add a test that fires two concurrent `Run` calls on the same runner and asserts `maxInFlight <= concurrency`.

- **Active autopin configuration is not observable via the management API.** `AUTOPIN_CONCURRENCY`, `AUTOPIN_AUTHOR_CAP`, `MAX_PIN_BYTES`, and `REFRESH_INTERVAL` are operator-tunable but none are reflected in any `/api` response. An agent or operator who did not start the process cannot read back the enforced limits without parsing startup logs. Extend `StatusResponse` with the four values and populate them in `handleStatus`. While there, populate `StatusResponse.Mode` (currently declared in the struct but never assigned, so it always serializes as `""`) — drive it from the backend type (`embedded` or `pinata`).

### Safe_auto cleanups already applied by the architect in the re-review commit

- Inlined `sumValues` at its single call site in `autopin_runner.go` and removed the helper. Cross-reviewer agreement on premature abstraction; no behavior change.
- Tightened `TestAutoPinRunnerRespectsConcurrencyBound` to assert `maxInFlight == 4` instead of the prior `>= 1 && <= 4` range, so a regression that accidentally halves the semaphore is caught instead of silently passing the upper-bound check.

### Out of scope for this task (recorded so the next architect pass does not re-discover them)

- **Sybil-author multi-account broadcast still wedges discovery for hours.** The per-author cap defends against single-author wedges but not against N attacker-controlled Hive accounts each broadcasting up to `AUTOPIN_AUTHOR_CAP` CIDs. The brainstorm explicitly rejected the global queue-depth backstop under a single-author threat model; the Sybil scenario reopens that decision. Treat as a follow-on task (e.g., `pinner-autopin-sybil-batch-cap`) if/when accredited-author identity binding is no longer assumed strong enough. Documented here as residual risk only.
